...
exports.findProjectByIdSearchPeriod = async (req, res) => {
  const { id } = req.params;
  const { periodName } = req.query; // Get the project code from query parameters
  ...
  const periodData = await prisma.period.findMany({
    ...
    where: { projectid: Number(id), periodStatusId: 2,description:{
      contains:periodName
    } },
  });
