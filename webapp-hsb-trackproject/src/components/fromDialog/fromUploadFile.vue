...
<template>
  <Dialog title="Upload File(s)" :closeOnEscape="false" modal :header="'c'"
      v-model:visible="dialogVisible" @update:visible="onCloseDialog" :draggable="false" :style="{ width: '50vw' }"
      :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
      <div class="card">
          <FileUpload name="files[]" @uploader="onUploadFiles" @select="onFileSelected" :multiple="true"
              :choose-label="'d'" :upload-label="'hh'"
              :cancel-label="'d'" accept="image/*" :maxFileSize="1000000" customUpload>
